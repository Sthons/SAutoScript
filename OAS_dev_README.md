...
4、脚本任务的组成
oas在设计任务有着严格的一套设计范式，具体的三大核心为用户选项、控制设备和过程元素。在这里做一个简要介绍：

用户选项表征用户对于该配置的自定义的设置，这些选项允许用户在特定情况下进行选择。这些选项可以以string、int、float等等形式出现，供用户选择合适的选项来影响脚本的执行过程或或者用户配置。
